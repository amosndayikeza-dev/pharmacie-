...
/**
 * Validation pour la modification d'une vaccination (API).
 */
class UpdateVaccinationRequest extends FormRequest
{
    ...
    {
        return [
            'animal_id'             => ['sometimes', 'required', 'integer', 'exists:animaux,id'],
            'veterinaire_id'        => ['nullable', 'integer', 'exists:veterinaires,id'],
            'medicament_id'         => ['nullable', 'integer', 'exists:medicaments,id'],
            'nom_vaccin'            => ['sometimes', 'required', 'string', 'max:150'],
            'numero_lot_vaccin'     => ['nullable', 'string', 'max:100'],
            'date_vaccination'      => ['sometimes', 'required', 'date', 'before_or_equal:today'],
